    ...
    if (!done) {
        ...
        if (index <= title.length) {
            ...
            if (title[index] === "<") {
                ...
                error_title.innerHTML = title.substr(0, counter) + "|";
                index = counter;
            } else {
                error_title.innerHTML = title.substr(0, index++) + "|";
            }
            setTimeout(() => {
                write_title(error);
            }, SPEED)
    ...
    if (currentLine === "description") {
        if (index <= text[error].description.length) {
            ...
            if (text[error].description[index] === "<") {
                ...
                description_line.innerHTML = text[error].description.substr(0, counter) + "|";
                index = counter;
            } else {
                description_line.innerHTML = text[error].description.substr(0, index++) + "|";
            }
            setTimeout(() => {
                next_letter(error);
            }, SPEED)
        ...
        if (index <= text[error].links.length) {
            ...
            if (text[error].links[index] === "<") {
                ...
                link_line.innerHTML = text[error].links.substr(0, counter) + "|";
                index = counter;
            } else {
                link_line.innerHTML = text[error].links.substr(0, index++) + "|";
            }
            setTimeout(() => {
                next_letter(error);
            }, SPEED)
...
function main(error) {
    ...
    if (params.prev) {
        previousPage = params.prev;
        console.log(previousPage);
    } else {
        previousPage = "/";
    }

    // This needs to be defined after the previousPage is set
    text["403"] = {
        "description": "You are not allowed to view this page",
        "links": `You could try <a href="${previousPage}">going back</a> or <a href="/">going home</a>`
    };
    text["404"] = {
