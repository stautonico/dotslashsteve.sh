...
export class ArgParser {
  ...
  constructor(options: ArgParserOptions) {
    ...
    for (let key in options.args) {
      ...
      if (arg.positional && (arg.long || arg.short)) {
        throw new Error(
          `ArgParser: Argument ${key} cannot both be a flag and positional`,
        );
      }
      ...
      if (arg.positional === false && !(arg.long || arg.short)) {
        throw new Error(
          `ArgParser: Argument ${key} must be a flag or positional`,
        );
      }
      ...
      if (arg.default && arg.type) {
        if (typeof arg.default !== arg.type) {
          throw new Error(
            `ArgParser: Argument ${key} default type ${typeof arg
              .default} does not match type ${arg.type}`,
          );
        }
      }
      ...
      if (arg_obj.positional) {
        ...
        if (short.length + long.length > 1) {
          if (arg_obj.type === "boolean") {
            DESCRIPTION += `\t\\e[0;32m${
              short.concat(long).join(" ")
            }\\e[0m\n\t\t${arg_obj.description}\n\n`;
          } else {
            DESCRIPTION += `\t<b>${
              short.concat(long).join(" ")
            }</b>=${arg_name.toUpperCase()}\n\t\t[PUT HELP STRING HERE]\n\n`;
          }
        } else {
          if (arg_obj.type === "boolean") {
            DESCRIPTION += `\t\\e[0;32m${
              short.concat(long).join(" ")
            }\\e[0m\t${arg_obj.description}\n\n`;
          } else {
            DESCRIPTION += `\t<b>${
              short.concat(long).join(" ")
            }</b>=${arg_name.toUpperCase()}\n\t\t[PUT HELP STRING HERE]\n\n`;
  ...
  public parse(argv: string[] | string): ArgParseResult {
      ...
      if (arg.startsWith("-")) {
        ...
        if (arg_obj.type === "boolean") {
          ...
          if (argv[i + 1] && !argv[i + 1].startsWith("-")) {
            ...
            this._custom_console_log(
              `${this._name}: Missing value for flag ${arg}`,
            );
          }
        } else {
          // We should never get here
          this._custom_console_log(
            `${this._name}: Unknown type ${arg_obj.type}`,
          );
          return new ArgParseResult({}, true);
        }
        ...
        if (expected_positional_arg === undefined) {
          this._custom_console_log(
            `${this._name}: Unexpected argument '${arg}'`,
          );
    ...
    if (missing_arg) {
      this._custom_console_log(
        `${this._name}: Missing required argument '${missing_arg}'`,
      );
      return new ArgParseResult({}, true);
    }
