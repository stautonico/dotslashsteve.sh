...
import {
    proc_uptime_read_handler,
    termprefs_write_handler,
} from "../util/callbacks";
...
export class FSBaseObject {
    ...
    protected events: {
        [key: number]: { event: EventType; callback: (file: FSBaseObject) => void };
    } = {};
    ...
    constructor(
        ...
        owner: number,
        group_owner: number,
        other?: FSBaseObjectOtherOptions,
    ) {
        this.name = name;
        this.owner = owner;
    ...
    delete_self(): Result<void> {
        ...
        if (this.parent !== undefined) {
            ...
            if (
                this.check_permissions("read").ok() &&
                this.check_permissions("write").ok()
            ) {
                ...
                return new Result({
                    success: false,
                    message: ResultMessages.NOT_ALLOWED,
                });
            }
        }
        return new Result({success: false, message: ResultMessages.NOT_ALLOWED});
    ...
    add_event_listener(
        event: EventType,
        callback: (file: FSBaseObject) => void,
        _when: "before" | "after" = "after",
    ): Result<number> {
...
export class File extends FSBaseObject {
    ...
    read(): Result<string> {
        ...
        return new Result({
            success: false,
            message: ResultMessages.NOT_ALLOWED_READ,
        });
    }
    ...
    write(content: string): Result<void> {
        ...
        if (true) {
            ...
            return new Result({
                success: false,
                message: ResultMessages.NOT_ALLOWED_WRITE,
            });
        }
    }
...
export class StandardFS {
    ...
    setup_etc(): void {
        ...
        let _passwd = new File("passwd", 0, 0, {
            parent: etc,
            content: "root:x:0:0:root:/root:/bin/bash\n",
        });
        let _shadow = new File("shadow", 0, 0, {
            parent: etc,
            content: "root:x/X.X.X.X.:16000:0:99999:7:::\n",
            permissions: {
    ...
    setup_home(): void {
        ...
        // @ts-ignore
        let users_home = new Directory("user", 1000, 1000, {
            parent: this.root.get_child("home").get_data(),
        });
        ...
        let termprefs_file = new File(".term_prefs", 1000, 1000, {
            parent: users_home,
        });
        ...
        let local_share_dir = new Directory("share", 1000, 1000, {
            parent: local_dir,
        });
        let local_share_fonts_dir = new Directory("fonts", 1000, 1000, {
            parent: local_share_dir,
        });
    ...
    async setup_usr(): Promise<void> {
        ...
        for (let file of bin_directory.get_children()) {
            ...
            try {
                ...
                new File(file_name, 0, 0, {parent: man, content: manpage});
            } catch (e) {
    ...
    find(path: string | Path): Result<FSBaseObject> {
        ...
        if (path === ".") {
            return new Result({
                success: true,
                data: computer.current_session().get_current_dir(),
            });
        }
        ...
        if (path === "..") {
            return new Result({
                success: true,
                data: computer.current_session().get_current_dir().get_parent(),
            });
        }
        ...
        let current_dir = path[0] === "/"
            ? this.root
            : computer.current_session().get_current_dir();
