...
export class Computer {
    ...
    new_session(uid: number): boolean {
        ...
        if (find_home_dir.fail()) {
            ...
            this.sessions.push(
                new Session({
                    real_uid: uid,
                    current_dir: find_home_dir.get_data()! as Directory,
                }),
            );
            return true;
        }
    ...
    sys$mkdir(pathname: string, _mode: number): Result<void> {
    ...
    sys$chdir(path: string): Result<void> {
        ...
        if (find_result.get_data()?.is_file()) {
            ...
            return new Result({
                success: false,
                message: ResultMessages.MISSING_ARGUMENT,
            });
        }

        this.sessions[this.sessions.length - 1].set_current_dir(
            // @ts-ignore: Handled by function above
            find_result.get_data(),
        );
        return new Result({success: true});
    }

    sys$open(path: string, flags: number, mode: number): Result<number> {
        let find_result = this.fs.find(path);

        if (find_result.fail()) {
            // If we are creating a file, do so
            if (flags & open_mode.O_CREAT) {
                // TODO: Check permissions
                // Try to find the parent directory
                let parent_path = path.split("/").slice(0, -1).join("/");
                let find_parent = this.fs.find(parent_path);
                if (find_parent.fail()) {
                    this.errno = Errno.ENOENT;
                    return new Result({success: false, message: ResultMessages.NOT_FOUND});
                ...
                // TODO: Use egid
                // @ts-ignore
                new File(filename, this.sys$geteuid(), this.sys$geteuid(), {parent: find_parent.get_data()});
                return new Result({success: true, data: 0});
